...
from sqd_lattice.sqd import load_completed_batches,load_iteration_from_checkpoint,save_batch_checkpoint,filter_addresses,make_hf_bitstring
...
print(f"Running SQD for {material} {ne} electrons and {samples_per_batch} samples per batch",flush=True)
...
iterations = sqd_config["recovery_iterations"]
n_batches = sqd_config["n_batches"]
num_cpus = sqd_config["num_cpus"]
checkpoint_interval = n_batches
max_davidson_cycles = sqd_config["max_davidson_cycles"]
sampling = sqd_config["sampling"]

# Defining results and checkpoints files
results_folder = os.path.join(folder_path,f"results_sqd_{sqd_config['sampling']}")
results_full_folder = os.path.join(results_folder,"results_full")
os.makedirs(results_full_folder,exist_ok=True)
checkpoint_file = os.path.join(results_full_folder,f'results_{samples_per_batch}_checkpoint.h5')
results_file = os.path.join(results_full_folder,f'results_{samples_per_batch}.h5')

# Loading QC counts
counts = json.load(open(os.path.join(folder_path,"counts",f"counts_{sampling}.json"),"r"))

print(f"Using counts obtained from {sampling}")
...
for i in range(iterations):
    ...
    if occupancies_bitwise is None:
        ...
        tf = time.time()
        print(f"Configuration recovery for iteration {i} completed in {(tf - ti)/3600} hours")
    ...
    for j in range(0, n_batches, checkpoint_interval):
        ...
        tf_total = time.time()
        print(f"Iteration {i} finished! Total computation time: {(tf_total - ti_total)/3600}",flush=True)
...
print(f"Minimal energy subspace dimension: {ci_coeffs_list[minimal_idx].shape[0]*ci_coeffs_list[minimal_idx].shape[1]}",flush=True)
