...
ne = args.ne
select_cutoff_idx = int(args.select_cutoff_idx)
...
print(f"Starting HCI {max_iter} iterations for {material} {sum(num_elec_ab)} electrons",flush=True)
print(hci_config)
...
energy_sci, state_sci, avg_occs = solve_hci(
    ...
    ci_strs=None,
    spin_sq=None,
    energy_tol=hci_config["energy_tol"],
    mpirun_options=["-quiet", "-n", f"{hci_config['num_cpus']}"],
    select_cutoff=select_cutoff_list[select_cutoff_idx],
    max_iter=max_iter,)
